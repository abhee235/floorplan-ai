...
import { bomToExportCsv, bomToXlsx, provenanceFor, sourceLookup } from "@fpv/exporters";
...
export const exportTool = defineTool({
  ...
  async run(args, call) {
    ...
    if (args.format !== "csv" && args.format !== "xlsx")
      throw unavailable(
        "export",
        `${args.format} export arrives in a later phase (glb in phase 2, pdf and dxf in phase 3)`,
        "use csv or xlsx for the bill of materials",
      );
    if (!ctx.rules)
      throw unavailable(
        "export",
